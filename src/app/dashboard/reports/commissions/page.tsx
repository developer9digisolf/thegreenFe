...
export default function CommissionsReportPage() {
  ...
  const [selectedEmployee, setSelectedEmployee] = useState<ICommissionByEmployee | null>(null);
  const [sessions, setSessions] = useState<ICommissionSession[]>([]);
  const [loadingSessions, setLoadingSessions] = useState(false);
  const [sessionPagination, setSessionPagination] = useState({ current: 1, pageSize: 10, total: 0 });
  ...
  const handleExport = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `Commission_Report_${dayjs().format('YYYYMMDD')}.xlsx`);
  ...
  const sessionColumns = [
    {
      ...
      render: (code: string, record: ICommissionSession) => (
        <div>
          ...
          <div className="text-[10px] text-slate-400">{dayjs(record.sessionDate).format("DD MMM YYYY")}</div>
        </div>
      ),
    },
    {
      ...
      render: (text: string, record: ICommissionSession) => (
        <div>
          ...
          <div className="text-xs text-slate-400">{record.serviceVariantName}</div>
        </div>
      ),
    },
    {
      ...
      key: "memberName",
      render: (text: string) => <Text className="text-sm">{text || "Guest"}</Text>,
    },
    {
      title: "Komisi",
    ...
    },
    {
      ...
      render: (status: string) => {
        ...
        return <Tag color={color} className="text-[10px] uppercase font-bold">{status}</Tag>;
      },
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <Title level={2} className="m-0 text-slate-800 font-extrabold tracking-tight">Laporan Komisi</Title>
          <Text className="text-slate-500">Monitor performa dan komisi karyawan Anda</Text>
        </div>
        <Space>
          <Button 
      ...
      <Card className="rounded-2xl shadow-sm border-none overflow-visible">
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex flex-col gap-1">
            <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Filter Tipe</Text>
            <Select
          ...
          <div className="flex flex-col gap-1">
            <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">
              {dateFilterType === "Day" ? "Pilih Tanggal" : 
               dateFilterType === "Week" ? "Pilih Minggu" : 
               dateFilterType === "Month" ? "Pilih Bulan" : "Pilih Rentang"}
            </Text>
            {dateFilterType === "Custom" ? (
              ...
              <DatePicker 
                className="h-10 rounded-xl"
                picker={dateFilterType === "Month" ? "month" : dateFilterType === "Week" ? "week" : "date"}
                value={selectedDate}
          ...
          <div className="flex flex-col gap-1 flex-1 min-w-[200px]">
            <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Cari Karyawan</Text>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <SummaryCard 
          ...
          value={dateFilterType === "Custom" ? 
                 ...
                 selectedDate?.format(dateFilterType === "Month" ? "MMMM YYYY" : "DD MMM YYYY") || "-"} 
      ...
      <Drawer
        title={
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="text-lg font-bold text-slate-800">{selectedEmployee?.name}</div>
              <div className="text-xs text-slate-500">{selectedEmployee?.positionName}</div>
            </div>
          </div>
        }
        placement="right"
      ...
      >
        <div className="p-6 bg-slate-50 border-b border-slate-200">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
              <div className="text-[10px] text-slate-400 font-bold uppercase mb-1">Total Komisi</div>
              <div className="text-xl font-extrabold text-emerald-600">
                Rp {(selectedEmployee?.totalAmount || 0).toLocaleString("id-ID")}
              </div>
            </div>
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
              <div className="text-[10px] text-slate-400 font-bold uppercase mb-1">Sesi Terdaftar</div>
        ...
        <Table 
          ...
          pagination={{
            ...sessionPagination,
            size: "small",
            onChange: (page) => selectedEmployee && fetchSessions(selectedEmployee.id, page),
            className: "px-6 py-4",
          }}
