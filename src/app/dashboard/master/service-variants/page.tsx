...
export default function ServiceVariantsPage() {
  ...
  const [selectedItem, setSelectedItem] =
    useState<IBranchServiceVariant | null>(null);
  ...
  const fetchData = async (
    ...
    try {
      ...
      notification.error({
        ...
        description:
          err?.message || "Terjadi kesalahan saat memuat data varian layanan",
      });
    } finally {
      setLoading(false);
    }
      ...
      if (formType === "create") {
        ...
        if (res.success) {
          notification.success({
            title: "Varian berhasil ditambahkan ke cabang",
          });
          setOpenForm(false);
          fetchData(1);
        }
      } else if (selectedItem) {
        const res = await BranchServiceVariantUpdateService(
          selectedItem.id,
          payload,
        );
        if (res.success) {
          notification.success({ title: "Varian berhasil diperbarui" });
          setOpenForm(false);
          fetchData();
        }
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (res.success) {
        notification.success({
          title: "Varian berhasil dihapus dari cabang",
        });
  ...
  const handleToggleStatus = async (id: number, checked: boolean) => {
    ...
    try {
      ...
      // Update with all required fields but only change isActive
      const res = await BranchServiceVariantUpdateService(id, {
        branchId: currentItem.branchId,
        serviceVariantId: currentItem.serviceVariantId,
      ...
      notification.error({
        ...
        description:
          err?.message || "Terjadi kesalahan saat mengubah status varian",
      });
    } finally {
  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      width: "100px",
      align: "center",
      render: (v: boolean, record: IBranchServiceVariant) => (
        <Switch
          checked={v}
          ...
          { type: "divider" },
          {
            ...
            onClick: () =>
              setDeleteModal({
                open: true,
                id: record.id,
                label: `${record.serviceName} - ${record.serviceVariantLabel}`,
              }),
          },
        ];
        return (
    ...
    <div className="p-4 lg:p-8">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
        <div>
          ...
          >
            Varian Layanan per Cabang
          </Typography.Title>
          <Typography.Text className="text-slate-400 font-medium">
            Kelola durasi dan harga khusus layanan untuk setiap cabang
          </Typography.Text>
        </div>
        <div className="flex items-center gap-3">
          <Button
      ...
      <Card className="rounded-3xl border-none shadow-sm mb-6 bg-white/60 backdrop-blur-md">
        <Row gutter={[16, 16]} align="middle">
          ...
          <Col xs={24} md={8}>
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-black text-transparent uppercase tracking-widest ml-1 select-none">
                RESET
              </span>
              <Button
                icon={<ReloadOutlined />}
          ...
          <div className="flex items-center gap-4 p-2">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-xs text-slate-400 font-medium m-0 mt-1">
                Konfigurasi harga dan status varian di cabang
              </p>
            </div>
          </div>
        }
      ...
      >
        <Spin spinning={loading}>
          <UseForm form={forms} initialValues={{ sortOrder: 1 }}>
            <Row gutter={[24, 0]} className="mt-6">
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  ...
                  shouldUpdate={(prevValues, currentValues) =>
                    prevValues.serviceId !== currentValues.serviceId
                  }
                >
                  {({ getFieldValue }) => {
                ...
                <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 mt-2">
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-slate-400">
                      Aktifkan untuk menampilkan varian ini di POS cabang
                    </span>
                  </div>
              ...
              <Col
                span={24}
                className="mt-8 flex justify-end gap-3 pt-6 border-t border-slate-100"
              >
                <Button
                  size="large"
