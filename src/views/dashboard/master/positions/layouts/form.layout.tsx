...
import { IPropsFormPosition } from "@afx/interfaces/master/position.iface";
import {
  IActionPosition,
  IStatePosition,
} from "@afx/models/dashboard/master/positions.model";
import { useStore } from "@afx/store/core";
import { Col, Modal, Row, Spin, Typography, InputNumber } from "antd";
...
export function FormPosition(props: IPropsFormPosition) {
  const {
    state: { position },
    isLoading,
  ...
  const loading =
    isLoading("createPosition") || isLoading("updatePosition") || false;

  // SET FORM VALUES WHEN POSITION DATA IS AVAILABLE
  useEffect(() => {
    if (props?.open) {
  ...
  }, [props?.open, props?.formType, position, props.forms]);
  ...
  const renderHeader = () => (
    <div className="flex items-center gap-3 p-2">
      <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center text-emerald-600 shadow-sm border border-emerald-500/20">
        {props?.formType === "create" ? <Plus size={20} /> : <Briefcase size={20} />}
      </div>
      <div className="flex flex-col text-left">
        ...
        <p className="text-xs text-slate-400 font-medium m-0 mt-0.5">Lengkapi informasi data jabatan karyawan</p>
      </div>
    </div>
  );
          ...
          <Row gutter={[16, 8]}>
            <Col span={12}>
              ...
              >
                <UseInput
                  standart={false}
                  placeholder="Masukkan kode (contoh: POS001)"
            ...
            <Col span={24}>
              <UseFormItem
                ...
                label="Gaji Pokok"
                {...itemLayouts}
                rules={[{ required: true, message: "Field gaji pokok wajib diisi" }]}
              >
                <InputNumber
                  className="w-full h-[46px] flex items-center px-1 !rounded-xl !border-2 !border-gray-100 hover:border-emerald-400 focus:border-emerald-500"
            ...
            <Col span={24}>
              ...
              >
                <UseInputArea
                  standart={false}
                  disabled={props?.formType === "detail"}
                  placeholder="Masukkan deskripsi singkat jabatan..."
                  rows={4}
                />
              </UseFormItem>
            </Col>
            ...
            <Col span={24}>
              {props?.formType === "create" && (
                <div className="flex justify-end mt-6 border-t border-slate-100 pt-6">
                  ...
                  >
                    {loading ? "Processing..." : "Simpan Jabatan"}
                  </button>
                </div>
              )}
              {props?.formType === "detail" && (
