                    ...
                    <div className="flex items-center gap-4">
                        <div className={`w-16 h-16 rounded-2xl border border-slate-100 bg-white flex items-center justify-center overflow-hidden shadow-sm p-2 transition-all ${!pm.isActive ? 'grayscale opacity-40' : 'group-hover:scale-105'}`}>
                            <img 
                                ...
                                onError={(e) => { (e.target as HTMLImageElement).src = "https://placehold.co/100x100/f1f5f9/94a3b8?text=Payment"; }}
                            />
                        </div>
                        <div>
                            <Typography.Title level={5} className="!m-0 text-slate-800 font-bold tracking-tight">{pm.name}</Typography.Title>
                            <Typography.Text type="secondary" className="text-[10px] uppercase font-bold tracking-widest text-emerald-600/60">{pm.code}</Typography.Text>
                        </div>
                    </div>
                    <Badge status={pm.isActive ? "success" : "default"} text={pm.isActive ? "Aktif" : "Non-aktif"} className="text-[10px] font-bold" />
                </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl border border-slate-100 transition-colors hover:bg-slate-100">
                        <div className="flex items-center gap-2">
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${pm.isActive ? 'bg-emerald-500/10 text-emerald-600' : 'bg-slate-200 text-slate-400'}`}>{pm.isActive ? <CheckCircleOutlined /> : <CloseCircleOutlined />}</div>
                            <div>
                        ...
                        <Switch size="small" checked={pm.isActive} loading={updatingIds[pm.id]} onChange={(checked) => handleUpdate(pm.id, { isActive: checked })} />
                    </div>

                    <div className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl border border-slate-100 transition-colors hover:bg-slate-100">
                        <div className="flex items-center gap-2">
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${pm.isVisibleInPOS ? 'bg-blue-500/10 text-blue-600' : 'bg-slate-200 text-slate-400'}`}>{pm.isVisibleInPOS ? <EyeOutlined /> : <EyeInvisibleOutlined />}</div>
                            <div>
                        ...
                        <Switch size="small" checked={pm.isVisibleInPOS} loading={updatingIds[pm.id]} onChange={(checked) => handleUpdate(pm.id, { isVisibleInPOS: checked })} />
                    </div>
                </div>
    ...
    return (
        <div className="p-4 lg:p-8 max-w-[1600px] mx-auto">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-10">
                <div>
                    <Typography.Title level={2} className="!m-0 text-slate-800 font-extrabold tracking-tight">Master Metode Pembayaran</Typography.Title>
                    <Typography.Text className="text-slate-400 font-medium">Kelola berbagai pilihan pembayaran untuk pelanggan dan integrasi POS</Typography.Text>
                </div>
                <div className="flex items-center gap-3 p-3 bg-white rounded-2xl border border-slate-100 shadow-sm">
                    <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center text-emerald-600"><ShopOutlined className="text-xl" /></div>
                </div>
            </div>
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-32 bg-white rounded-[3rem] border border-slate-100 shadow-sm">
                    ...
                    <Typography.Text className="mt-6 text-slate-400 font-medium animate-pulse">Menyiapkan konfigurasi pembayaran...</Typography.Text>
                </div>
            ) : groups.length === 0 ? (
                <Empty description="Belum ada metode pembayaran yang terdaftar" image={Empty.PRESENTED_IMAGE_SIMPLE} className="py-32 bg-white rounded-[3rem] border border-slate-100" />
            ) : (
                <div className="space-y-12">
                    {groups.map(group => (
                        <div key={group.id} className="relative">
                            <div className="flex items-center gap-4 mb-6 px-2">
                                <div className="flex items-center justify-center w-12 h-12 bg-white rounded-2xl shadow-sm border border-slate-100 text-slate-800 font-bold text-lg">{group.sortOrder}</div>
                                <div>
                                    <Typography.Title level={4} className="!m-0 text-slate-800 font-extrabold">{group.name}</Typography.Title>
                                    <Typography.Text className="text-slate-400 text-xs font-medium">{group.description || `Grup pembayaran ${group.name.toLowerCase()}`}</Typography.Text>
                                </div>
                                <div className="flex-1 h-[1px] bg-gradient-to-r from-slate-200 to-transparent ml-4" />
                            </div>
                            <Row gutter={[24, 24]}>{group.paymentMethods.map(pm => renderPaymentMethod(pm))}</Row>
                        </div>
                    ))}
                </div>
            )}
            ...
            <style jsx global>{`
                .custom-spin-emerald .ant-spin-dot-item { background-color: #10b981 !important; }
