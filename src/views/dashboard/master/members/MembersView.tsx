...
export default function MembersView() {
  ...
  const handleOpenDetail = (member: IMember) => {
    router.push(`/dashboard/master/members/${member.id}`);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (formType === "create") {
        ...
        const res = await UpdateMemberService(
          selectedMember.id,
          payload as IUpdateMemberRequest,
        );
        if (res.success) {
  ...
  const columns: Column[] = [
    {
      ...
      width: "120px",
      render: (v: string) => (
        <code className="text-emerald-600 font-bold bg-emerald-50 px-2 py-1 rounded-lg">
          {v}
        </code>
      ),
    },
    ...
    },
    {
      ...
      render: (v: number) => (
        <div
          className={`font-bold ${v > 0 ? "text-emerald-600" : "text-slate-400"}`}
        >
          {formatCurrency(v)}
        </div>
      ),
    ...
    <div className="p-4 lg:p-8">
      ...
      <div className="bg-white rounded-[2.5rem] p-6 lg:p-8 shadow-[0_20px_50px_rgba(0,0,0,0.04)] border border-slate-100">
        <UseDynamicTable
          ...
          filters={
            <div className="flex items-center gap-3">
              <Typography.Text className="text-slate-400 font-medium whitespace-nowrap">
                Filter by:
              </Typography.Text>
              <Select
          ...
          <div className="flex items-center gap-4 p-2">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-xs text-slate-400 font-medium m-0 mt-1">
                Kelola informasi profil dan keanggotaan pelanggan
              </p>
            </div>
          </div>
        }
          ...
          <UseForm form={forms} initialValues={memberInitialValues}>
            <Row gutter={[24, 0]} className="mt-6">
              ...
              <Col span={24}>
                ...
                >
                  <UseInputArea
                    placeholder="Informasi tambahan tentang preferensi pelanggan..."
                    disabled={formType === "detail"}
                    rows={2}
                  />
                </UseFormItem>
