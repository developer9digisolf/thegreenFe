...
export default function BranchAdditionalCostsTab({ branchId, isOpen }: Props) {
  ...
  const [selectedItem, setSelectedItem] =
    useState<IBranchAdditionalCost | null>(null);
  ...
  const handleSave = async (values: any) => {
    try {
      ...
      if (modalMode === "create") {
        ...
        const res = await UpdateBranchAdditionalCostService(
          selectedItem.id,
          branchId!,
          payload,
        );
        if (res.success || res.meta?.success)
          antdNotification.success({
            message: "Biaya tambahan berhasil diperbarui",
          });
      }
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      if (res.success || res.meta?.success) {
        antdNotification.success({
          message: "Berhasil menghapus biaya tambahan",
        });
        fetchData();
      }
    } catch (err: any) {
      ...
      <div className="flex justify-between items-center mb-2">
        <div>
          <Typography.Title level={5} className="m-0">
            Biaya Tambahan Cabang
          </Typography.Title>
          <Typography.Text className="text-slate-500 text-xs">
            Kelola pajak, layanan, atau biaya admin khusus cabang ini
          </Typography.Text>
        </div>
        <Button
          type="primary"
            ...
            render: (_, record) => (
              <Space orientation="vertical" size={0}>
                <Typography.Text strong>
                  {record.additionalCost?.name}
                </Typography.Text>
                <Typography.Text className="text-xs text-slate-500">
                  {record.additionalCost?.code}
                </Typography.Text>
              </Space>
            ),
          },
          {
            ...
            render: (_, record) => (
              <span className="font-bold text-emerald-600">
                {record.additionalCost?.isPercentage
                  ? `${record.price}%`
                  : formatUang(record.price)}
              </span>
            ),
          },
          {
              ...
              >
                {masterCosts
                  .filter((c) => {
                    ...
                    return !data.some(
                      (existingItem) => existingItem.additionalCostId === c.id,
                    );
                  })
                  .map((c) => (
